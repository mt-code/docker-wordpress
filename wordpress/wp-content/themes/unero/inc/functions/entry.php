 ...
 */
function unero_instagram_photos( $numbers, $title, $columns, $autoplay, $instagram_access_token = false ) {
	...
	$results = instagram_get_photos_by_token($numbers,$instagram_access_token);
	...
	if ( is_wp_error( $results ) ) {
			echo wp_kses_post( $results->get_error_message() );
		} elseif ( $results ) {

		foreach ( $results as $item ) {

			$image_link = $item['images']['thumbnail'];
			$image_url  = $item['link'];
...
*/
function instagram_get_photos_by_token( $limit,$access_token ) {

	if ( empty( $access_token ) ) {
		return new WP_Error( 'instagram_no_access_token', esc_html__( 'No access token', 'unero' ) );
	}
	$user = unero_get_instagram_user($access_token);
	if ( ! $user || is_wp_error( $user ) ) {
		return $user;
	}
	if (isset($user['error'])) {
		return new WP_Error( 'instagram_no_images', esc_html__( 'Instagram did not return any images. Please check your access token', 'unero' ) );
	} else {
		$transient_key = 'unero_instagram_photos_' . sanitize_title_with_dashes( $user['username'] . '__' . $limit );
		...
		while ( count( $images ) < $limit ) {
			if ( ! $next ) {
				$fetched = unero_fetch_instagram_media( $access_token );
			} else {
				$fetched = unero_fetch_instagram_media( $next );				
			}
		...
		if ( ! empty( $images ) ) {
			...
			return new WP_Error( 'instagram_no_images', esc_html__( 'Instagram did not return any images.', 'unero' ) );
		}
	}
}
/**
...
* @return array
*/
function unero_fetch_instagram_media( $access_token ) {
...
*/
function unero_get_instagram_user($access_token) {
	if ( empty( $access_token ) ) {
		return new WP_Error( 'no_access_token', esc_html__( 'No access token', 'unero' ) );
	}
	$user = get_transient( 'unero_instagram_user_' . $access_token );
	if ( false === $user ) {
		$url  = add_query_arg( array( 'fields' => 'id,username', 'access_token' => $access_token ), 'https://graph.instagram.com/me' );
		$data = wp_remote_get( $url );
		$data = wp_remote_retrieve_body( $data );
		if ( ! $data ) {
			return new WP_Error( 'no_user_data', esc_html__( 'No user data received', 'unero' ) );
		}
		$user = json_decode( $data, true );
		if ( ! empty( $data ) ) {
			set_transient( 'unero_instagram_user', $user, 2592000 ); // Cache for one month.
		}
	}
	return $user;
}